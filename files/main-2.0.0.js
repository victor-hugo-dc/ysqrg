...
$('.load-posts').on('click', function (e) {
  ...
  $.ajax({
    ...
    success: function success(response) {
      current_page++;
      var grid = $('#posts-list');
      grid.append($(response).find('#posts-list article'));
      if (current_page >= load.total) {
  ...
  if (!global.suppress || !$blogList.parents('.editable').length) {
    $.ajax({
      ...
      success: function success(response) {
        ...
        if (response.length) {
          var $injectArea = $blogList.find('.posts-wrapper');
          $injectArea.find('.loading').hide();
          ...
          response.forEach(function (item) {
            ...
            $injectArea.append(article);
          });
        } else {
          var $injectArea = $blogList.find('.posts-wrapper');
          $injectArea.find('.loading').text('No Posts Found');
        }
      },
      error: function error(_error) {
        console.log(_error);
        var $injectArea = $blogList.find('.posts-wrapper');
        $injectArea.find('.loading').text(_error);
      }
    });
  }
});
...
},{"lodash":1,"moment":2}],6:[function(require,module,exports){
'use strict';
...
if ($('.has-fixed-header.transparent-header').length) {
...
$('.menu-toggle').on('click', function (e) {
  ...
  if ($(this).hasClass('open')) {
    $('#main-navigation').velocity('slideUp', 500, 'easeOutExpo');
  } else {
    $('#main-navigation').velocity('slideDown', 500, 'easeOutExpo');
  }
...
$('.tabs-nav a[data-tab]').on('click', function (e) {
  ...
  history.pushState({ slug: tabHref }, null, '/' + tabHref);
...
},{"./blogs":5,"./navigation":7,"velocity-animate":3}],7:[function(require,module,exports){
'use strict';
...
function scrollToSection(slug) {
  ...
  if ($('#section-' + slug + ' .main-content-wrapper').length && slug !== 'home') {
    $('#section-' + slug + ' .main-content-wrapper').velocity('scroll', scrollSettings);
...
function navigation() {
  ...
  window.addEventListener('popstate', function (e) {
    ...
    $('#main-navigation li').removeClass('active');
    $('#main-navigation a[data-section="' + slug.split('/')[0] + '"]').parent('li').addClass('active');
    scrollToSection(slug.split('/')[0]);

    if (slug.split('/')[1]) {

      var $tab = $('.tabs-nav a[data-tab="' + slug + '"]'),
          tabHref = $tab.data('tab'),
          $tabsWrapper = $tab.closest('.section-tabs').find('.tabs-content'),
          newTabHeight = $tabsWrapper.find('.tab-pane[data-tab="' + tabHref + '"]').outerHeight();
...
},{"velocity-animate":3,"waypoints":4}]},{},[6]);
